...
**Concept:** Portsmith's `sensor_readings` table (Chapter 8) has grown
past 9.6 million rows, and the questions being asked of it have
shifted from "record this one reading" to "what's the citywide
average?" — exactly the shape of question row-organized storage
answers slowly, since it has to read every column of every row to get
there. `pgColumnar` adds a second way to store a table, organized by
column instead, directly inside PostgreSQL. Written and verified live
against a real `pgColumnar` 1.0-alpha3 build: real, measured wins —
708 MB down to 18 MB (40x smaller), `count(*)` from 562.7ms down to
0.065ms (~8,700x, answered from metadata alone) — plus a genuine echo
...
**Exercises:**
1. Create a column-organized copy of `sensor_readings` with `CREATE
   TABLE ... USING pgcolumnar`, and measure real storage: 708 MB heap
   down to 18 MB columnar, ~40x smaller, for identical data.
2. The easy win: an unfiltered `count(*)` drops from 562.7ms to
   0.065ms (~8,700x) — answered entirely from stored metadata, no
   scanning at all.
3. A realistic, filtered query (average reading for one sensor type)
   is *slower* on the columnar copy at first (1459.5ms vs. the heap's
   486.6ms) — the same scattered-data problem Chapter 20 found for
   `sensor_id`, this time in `pgColumnar`'s own zone maps.
4. Fix it with `pgcolumnar.vacuum_sorted()` (370.3ms, now faster than
...
- **F — Syntax Quick Reference:** A per-chapter cheatsheet of the core commands/operators/functions this book actually used — JSONB operators (Ch1), PostGIS functions (Ch2), `FOR UPDATE SKIP LOCKED` (Ch3), `tsvector`/`tsquery` (Ch4), `pg_trgm` operators (Ch5), `pgvector` distance operators (Ch6), `ip4r` operators (Ch7), partitioning DDL (Ch8), `REFRESH MATERIALIZED VIEW` forms (Ch9), PostgREST/RLS grant patterns (Ch10), window function frame syntax (Ch11), `WITH RECURSIVE`/`CYCLE` (Ch12), `LISTEN`/`NOTIFY` (Ch13), advisory lock functions (Ch14), domain/enum/composite DDL (Ch15), generated column syntax (Ch16), FDW DDL (Ch17), logical replication DDL (Ch18), `cron.schedule` forms (Ch19), `EXPLAIN`/`pg_stat_statements` queries (Ch20), `CREATE PROPERTY GRAPH`/`GRAPH_TABLE` (Ch21), SPARQL/SHACL/`pg_ripple` function signatures (Ch22-23), and `pgcolumnar` storage/Parquet functions (Ch24) — one page per chapter, meant to be flipped to rather than read start to end, and a natural place to call out which syntax this book verified working versus verified broken (Ch21's unsupported quantifiers, Ch22's rule-chaining bug, Ch23's `infer()` data-corruption warning) so a reader skimming for syntax doesn't copy something this book already found doesn't work.
- **G — Acknowledgments:** A direct thank-you to every piece of software this book depends on and didn't write — PostgreSQL itself, every extension used chapter by chapter, the Python libraries behind the data-generation scripts, the Markdown/Pandoc/Mermaid/Make toolchain that builds this book, the open standards Chapters 22-23 are built on, and Anthropic's Claude Code, credited as the actual collaborator it was throughout. Logos included where a project has one freely available to credit it with.
