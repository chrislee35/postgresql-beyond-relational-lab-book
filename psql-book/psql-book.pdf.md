...
<img src="imgs/ch01_null_vs_null.png" alt="Flowchart: does the JSONB key exist? If not, SQL NULL. If it exists, what's the value — JSON null (the gotcha: IS NOT NULL is still TRUE) or a real object (IS NOT NULL is TRUE, correctly)"/>
...
<img src="imgs/portsmith_map.png" alt="Map of Portsmith showing its six neighbourhoods, six parks, road network, and business locations, rendered directly from the PostGIS data this chapter loads"/>
...
<img src="imgs/ch03_job_lifecycle.png" alt="State diagram: queued transitions to in_progress on claim; in_progress transitions to completed on worker success, back to queued on a reclaim sweep if attempts are below max_attempts, or to dead_letter_jobs on a reclaim sweep if attempts are exhausted"/>
...
<img src="imgs/ch03_skip_locked.png" alt="Sequence diagram: with FOR UPDATE SKIP LOCKED, Session A claims rows 1-5 and Session B immediately claims rows 6-10 with neither session blocking; with plain FOR UPDATE, Session B blocks after requesting rows 1-5 until Session A commits, then receives the same rows 1-5"/>
...
<img src="imgs/ch04_search_pipeline.png" alt="Flowchart: raw text flows through tokenize (split into words, lowercase, strip punctuation), then stem (reduce to root form), then filter stopwords (discard low-information words), producing a tsvector"/>
...
<img src="imgs/ch05_trigram_window.png" alt="A 3-character window sliding one position at a time across the padded string '  eleanor ', producing the 8 overlapping trigrams: '  e', ' el', ele, lea, ean, ano, nor, 'or '"/>
...
<img src="imgs/ch06_ivfflat_clustering.png" alt="Diagram of five vector clusters. The query vector sits inside cluster A, which probes=1 searches. The true nearest neighbor actually sits just across the boundary in adjacent cluster E, which is never searched, so it's missed entirely"/>
...
<img src="imgs/ch07_cidr_nesting.png" alt="Number-line diagram showing 203.0.113.0/24 as a wide range and 203.0.113.128/26 as a narrower range nested inside it, with the address 203.0.113.150 landing inside both at once"/>
...
<img src="imgs/ch08_partition_pruning.png" alt="Timeline of the 12 monthly partitions from 2024_01 through 2024_12. Only 2024_06 is highlighted as scanned, matching the WHERE clause's June date range; the other eleven are greyed out as pruned"/>
...
<img src="imgs/ch08_subpartitioning.png" alt="Partition tree: sensor_readings_by_type partitioned by LIST(sensor_type) into three branches (temperature, traffic, air_quality), each further partitioned by RANGE(recorded_at) into three monthly leaf partitions"/>
...
<img src="imgs/ch12_cycle_detection.png" alt="Directed chain: Priya Nair at depth 0 points to Dana Ruiz at depth 1, who points to Leo Park at depth 2, who points back to Dana Ruiz at depth 3 — this final repeated node is flagged is_cycle = true, where CYCLE stops the walk instead of looping forever"/>
...
<img src="imgs/ch12_road_graph.png" alt="Map of the relevant slice of Portsmith's road graph: a shared black path runs from Fisherman's Row & Market Street through Lighthouse Avenue to Bay Street & Ring Road (West); from there the fewest-hops route (red) follows Ring Road's long bend north and around to Bay Street & Ring Road (East), while the shortest-distance route (teal) cuts directly across via Bay Street & Canal Road, arriving nearly 5 kilometres sooner despite one extra hop"/>
...
<img src="imgs/ch15_type_comparison.png" alt="Three type-system tools compared: ENUM creates a type that holds a fixed set of labels and nothing else; DOMAIN attaches a reusable constraint to an existing base type; COMPOSITE TYPE bundles several named fields into one column."/>
...
<img src="imgs/ch15_reflection_flow.png" alt="One job_status entry in the pg_enum catalog, fanning out to two independent consumers: psycopg's register_enum() builds a live Python enum.Enum class from it, and PostgREST's OpenAPI document lists it as an enum array in the API schema — both automatically derived, neither hand-written"/>
...
<img src="imgs/ch16_pruning_vs_index.png" alt="Timeline of all twelve sensor_readings partitions, none of them greyed out or pruned — every partition gets its own index scan for a reading_date filter, with 2024_06 highlighted as the one that actually matches, in direct contrast to Chapter 8's diagram where filtering by the partition key pruned eleven of twelve partitions before execution"/>
...
<img src="imgs/ch16_write_paths.png" alt="Two write paths compared: a trigger-maintained column depends on the trigger still being attached and enabled, and silently goes stale if it's disabled, dropped, or the column is written to directly; a generated column recomputes automatically from any write to the row and structurally rejects any direct write, with no other path to go stale through"/>
...
<img src="imgs/logo_postgresql.png" alt="PostgreSQL elephant logo" width="90" style="width:90px;height:auto;"/>
...
- **DuckDB** <img src="imgs/logo_duckdb.png" alt="DuckDB logo" width="70" style="width:70px;height:auto;"/> — the independent, in-process analytical engine used to verify Chapter 17's Parquet export actually worked. [duckdb.org](https://duckdb.org/)
- **MinIO** <img src="imgs/logo_minio.png" alt="MinIO logo" width="70" style="width:70px;height:auto;"/> — the self-hosted, S3-compatible object storage Chapter 17's Parquet files lived in. [min.io](https://min.io/)

---

## Python and Its Libraries

<img src="imgs/logo_python.png" alt="Python logo" width="45" style="width:45px;height:auto;"/>
...
- **GNU Make** <img src="imgs/logo_gnu.png" alt="GNU logo" width="45" style="width:45px;height:auto;"/> — the build tool coordinating every step above, part of the GNU Project. [gnu.org/software/make](https://www.gnu.org/software/make/)
...
- **Debian** <img src="imgs/logo_debian.png" alt="Debian logo" width="45" style="width:45px;height:auto;"/> and **Docker** <img src="imgs/logo_docker.png" alt="Docker logo" width="70" style="width:70px;height:auto;"/> — the operating system and containerization underneath every isolated environment in Chapters 21, 22, and 24, letting three different PostgreSQL majors coexist on one machine without touching each other. [debian.org](https://www.debian.org/), [docker.com](https://www.docker.com/)

---

## Specifications and Open Standards

<img src="imgs/logo_apache.png" alt="Apache feather logo" width="40" style="width:40px;height:auto;"/>
...
<img src="imgs/logo_anthropic.png" alt="Anthropic logo" width="140" style="width:140px;height:auto;"/>
